...
void main() {
  group('VideoEditRequest', () {
    ...
    test('rejects empty inputPath', () {
      expect(
        VideoEditRequest(inputPath: '', outputPath: '/tmp/o.mp4').toMap,
        throwsArgumentError,
      );
    });
    ...
    test('rejects empty outputPath', () {
      expect(
        VideoEditRequest(inputPath: '/tmp/i.mp4', outputPath: '').toMap,
        throwsArgumentError,
      );
    });
    ...
    test('accepts all valid rotations', () {
      for (final deg in [0, 90, 180, 270]) {
        expect(
          ...
          returnsNormally,
          reason: 'rotation should be valid',
        );
      }
    });
  ...
  group('VideoCropRect', () {
    ...
    test('serializes valid crop rect to correct map', () {
      expect(
        ...
        <String, Object?>{'left': 0.1, 'top': 0.2, 'width': 0.5, 'height': 0.6},
      );
    });
    ...
    test('rejects zero width', () {
      expect(
        const VideoCropRect(left: 0, top: 0, width: 0.0, height: 0.5).validate,
        throwsArgumentError,
      );
    });
    ...
    test('rejects zero height', () {
      expect(
        const VideoCropRect(left: 0, top: 0, width: 0.5, height: 0.0).validate,
        throwsArgumentError,
      );
    });
    ...
    test('accepts exact 1.0 boundary (full frame)', () {
      expect(
        const VideoCropRect(left: 0, top: 0, width: 1.0, height: 1.0).validate,
        returnsNormally,
      );
    });
